...
class Game extends React.Component {
	...
	continueUpdate = (displayWarning, updateSyllables = true, updateDefinition = true) => {
		...
		const history = this.state.history.map((poem, i) => {
			if (i === this.state.currentPoemIndex) {
				...
				valid = this.validatePoem(poem.activeEdit);
		...
		this.updateSyllableCounts(currentPoem.activeEdit);
	...
	handleTitleChange = (newTitle) => {
		this.setState({
			history: this.state.history.map((poem, index) => {
				if (index === this.state.currentPoemIndex) {
	...
	validatePoem = (lines) => {
		...
		//const lines = currentPoem.activeEdit;
		console.log(`validatePoem`);
		console.log(`lines: ${JSON.stringify(lines)}`);
		const syllableCounts = this.updateSyllableCounts(lines);
		if (!lines) {
			return false;
		}
